...
e.addEventListener('click', () => { this.location = 'web3.html'; });
const ne = document.getElementById('n');
ne.addEventListener('click', () => { this.location = 'web5.html'; });

const mv = document.getElementById('m');
mv.addEventListener('ended', () => {
    let g = confirm('reproducir el siguente video ?');
    if (g) {
        mn.play();
    }
})
...
mh.addEventListener('ended', () => {
    let g = confirm('reproducir video anterior?');
    if (g) {
        mg.play()
    }
})